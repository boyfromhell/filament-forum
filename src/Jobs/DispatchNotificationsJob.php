...
use IchBin\FilamentForum\Notifications\EmailNotification;
        ...
        switch ($this->notification) {
                ...
                if ($this->object) {
                    ...
                    foreach ($users as $u) {
                        if ($this->user->id != $u->id) {
                            ...
                            $url = route('foru.discussion', ['discussion' => $this->object, 'slug' => Str::slug($this->object->name)]);
                            $recipient = $u;
                            $this->sendNotification($title, $body, $recipient, $url);
                        }
                    }
            ...
            case NotificationConstants::MY_REPLY_BEST_ANSWER->value:
                ...
                if ($this->object->user->id != $this->user->id) {
                    ...
                    $url = route('forum.discussion', ['discussion' => $this->object->discussion, 'slug' => Str::slug($this->object->name), 'r' => $this->object->id]);
                    $recipient = $this->object->user;
                    $this->sendNotification($title, $body, $recipient, $url);
                }
                break;
            case NotificationConstants::MY_POSTS_COMMENTED->value:
                ...
                if ($this->object->user->id != $this->user->id) {
                    if ($this->object->source instanceof Discussion) {
                        ...
                        $url = route('forum.discussion', ['discussion' => $this->object->discussion, 'slug' => Str::slug($this->object->name), 'd' => $this->object->source->id, 'c' => $this->object->id]);
                    } elseif ($this->object->source instanceof Reply) {
                        $title = 'Reply commented';
                        $body = $this->user->name . ' added a comment to your reply';
                        $url = route('forum.discussion', ['discussion' => $this->object->discussion, 'slug' => Str::slug($this->object->name), 'd' => $this->object->source->source->id, 'r' => $this->object->source->id, 'c' => $this->object->id]);
                    }
                    $recipient = $this->object->user;
                    $this->sendNotification($title, $body, $recipient, $url);
                }
            ...
            case NotificationConstants::MY_POSTS_LIKED->value:
                ...
                if ($this->object->source instanceof Discussion) {
                    ...
                    $url = route('forum.discussion', ['discussion' => $this->object->source, 'slug' => Str::slug($this->object->source->name), 'd' => $this->object->source->id, 'l' => $this->object->id]);
                    $recipient = $this->object->source->user;
                } elseif ($this->object->source instanceof Reply) {
                    $title = 'Reply liked';
                    $body = $this->user->name . ' liked your reply';
                    $url = route('forum.discussion', ['discussion' => $this->object->source->discussion, 'slug' => Str::slug($this->object->source->discussion->name), 'd' => $this->object->source->discussion->id, 'r' => $this->object->source->id, 'l' => $this->object->id]);
                    $recipient = $this->object->source->user;
                    ...
                    if ($this->object->source->source instanceof Discussion) {
                        $url = route('forum.discussion', ['discussion' => $this->object->source->source, 'slug' => Str::slug($this->object->source->source->name), 'd' => $this->object->source->source->id, 'c' => $this->object->source->id, 'l' => $this->object->id]);
                        $recipient = $this->object->source->source->user;
                    } elseif ($this->object->source->source instanceof Reply) {
                        $url = route('forum.discussion', ['discussion' => $this->object->source->source->discussion, 'slug' => Str::slug($this->object->source->source->discussion->name), 'd' => $this->object->source->source->discussion->id, 'r' => $this->object->source->source->id, 'c' => $this->object->source->id, 'l' => $this->object->id]);
                        $recipient = $this->object->source->source->user;
                    }
                }
                $this->sendNotification($title, $body, $recipient, $url);
