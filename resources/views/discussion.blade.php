<x-layout>
    <x-slot name="center">
        <livewire:header :discussion="$discussion"/>
        <!-- Page content -->
        <div class="w-full flex justify-center items-center py-10">
            <div class="container mx-auto flex lg:flex-row flex-col gap-10">
                <div class="flex flex-col lg:w-5/6 w-full lg:order-1 order-2 bg-white rounded-md shadow py-4">
                    <livewire:discussion-details :discussion="$discussion"/>
                    <livewire:replies :discussion="$discussion"/>
                </div>
...
//                        )
                        ...
                        <livewire:reply-btn :discussion="$discussion"/>
                    @endif
                    @if(
                        (auth()->user() && auth()->user()->hasVerifiedEmail())
//                        &&
//                        (auth()->user()->id != $discussion->user_id)
                        ...
                        <livewire:follow :discussion="$discussion"/>
                    @endif
                    @if(
                        (auth()->user() && auth()->user()->hasVerifiedEmail())
//                        &&
//                        auth()->user()->can(Permissions::LOCK_DISCUSSIONS->value)
                        ...
                        <livewire:lock :discussion="$discussion"/>
                    @endif
                    @if(
                        (auth()->user() && auth()->user()->hasVerifiedEmail())
//                        &&
...
//                        )
                        ...
                        <div class="w-full flex flex-col gap-1 justify-between text-sm text-slate-500">
                            <div>
                                <x-far-eye class="w-4 inline"/>
                                <span
                                    class="font-medium">Total views:</span> {{ $discussion->visits }}
                            </div>
                            <div>
                                <x-far-user class="w-4 inline"/>
                                <span
                                    class="font-medium">Unique views:</span> {{ $discussion->unique_visits }}
                            </div>
                        </div>
                    @endif
                </div>
        ...
        @if(auth()->user() && auth()->user()->hasVerifiedEmail())
            <!-- Add a reply modal -->
            <div id="add-reply-modal" data-modal-placement="bottom-center" data-modal-backdrop="static" tabindex="-1"
                 class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
                <div class="relative w-full h-full max-w-7xl md:h-auto">
                    <!-- Modal content -->
                    <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                        <button id="hide-reply-modal" type="button"
                                ...
                                data-modal-toggle="add-reply-modal">
