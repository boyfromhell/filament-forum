        ...
        <div class="w-full flex flex-col gap-0 bg-slate-50 border-y border-slate-100">
            @if($model->comments->count())
                @foreach($model->comments as $c)
                    <div
                        class="w-full flex flex-col py-5 px-3 gap-2 {{ $loop->last ? '' : 'border-b border-slate-200' }} hovered-section"
                        id="comment-{{ $c->id }}">
                        <div class="w-full text-slate-700 text-sm">
                            <a @if(auth()->user() && auth()->user()->hasVerifiedEmail()) href="{{ route('profile.show', ['username' => $c->user->username]) }}"
                               @else data-modal-toggle="login-modal"
                               @endif class="hover:cursor-pointer hover:underline font-medium">{{ $c->user->username }}</a>
                            (<span class="text-xs">{{ $c->created_at->diffForHumans() }}</span>) -
                            <span>{!! nl2br(e($c->content)) !!}</span>
                        </div>
...
//                                )
                                ...
                                <button type="button" class="flex items-center gap-2 hover:cursor-pointer">
                                    @if($c->likes()->where('user_id', auth()->user()->id)->count())
                                        <x-fas-thumbs-up class="w-4 text-red-500"
                                                         wire:click="toggleCommentLike({{ $c->id }})"/>
                                    @else
                                        <x-far-thumbs-up class="w-4" wire:click="toggleCommentLike({{ $c->id }})"/>
                                    @endif
                                    <span
                                        wire:click="selectComment({{ $c->id }})">{{ $c->likes->count() }} {{ $c->likes->count() > 1 ? 'Likes' : 'Like' }}</span>
                                </button>
                            @else
                                <div class="flex items-center gap-2">
                                    @if($c->likes()->where('user_id', auth()->user()->id)->count())
                                        <x-fas-thumbs-up
                                            class="w-4"/> {{ $c->likes->count() }} {{ $c->likes->count() > 1 ? 'Likes' : 'Like' }}
                                    @else
                                        <x-far-thumbs-up
                                            class="w-4"/> {{ $c->likes->count() }} {{ $c->likes->count() > 1 ? 'Likes' : 'Like' }}
                                    @endif
...
//                                )
                                ...
                                <button wire:click="editComment({{ $c->id }})" type="button"
...
//                                )
                                ...
                                <button wire:click="deleteComment({{ $c->id }})" type="button"
                    ...
                    <!-- Comment likes modal -->
                    <div id="comment-likes-modal-{{ $c->id }}" tabindex="-1" aria-hidden="true"
                        ...
                        <div class="relative w-full h-full max-w-md md:h-auto">
                            ...
                            <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                                <button type="button"
                                        ...
                                        id="hide-comment-likes-modal-{{ $c->id }}">
                                ...
                                <livewire:likes :model="$c" :wire:key="time() . '-comment-likes-' . $c->id"/>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        @if($comment)
            <form wire:submit="saveComment">
                ...
                <div class="flex items-center space-x-2 rounded-b mt-2">
                    ...
                    <button type="button" wire:click="cancelComment()" wire:loading.attr="disabled"
                            class="text-white bg-slate-700 disabled:bg-slate-300 hover:bg-slate-800 focus:ring-4 focus:outline-none focus:ring-slate-300 font-medium rounded text-sm px-5 py-2.5 text-center dark:bg-slate-600 dark:hover:bg-slate-700 dark:focus:ring-slate-800">
                        Cancel
                    </button>
                </div>
            </form>
...
//                )
                ...
                <button wire:click="addComment()" type="button"
                        class="bg-slate-100 px-3 py-2 text-slate-500 text-sm border-slate-100 rounded hover:cursor-pointer w-fit hover:bg-slate-200">
                    Add comment
                </button>
            @endif
        @endif
