<div class="w-full flex flex-col">
    <div class="w-full px-4 flex lg:flex-row flex-col lg:justify-between justify-start lg:items-center items-start gap-1">
    ...
    <div class="w-full flex flex-col" id="replies">
        @foreach($replies as $reply)
            <livewire:reply-details :reply="$reply" :key="time() . 'reply' . $reply->id" />
        @endforeach
    </div>
    @if(!$replies->count())
...
//            || auth()->user()->can(Permissions::REPLY_TO_DISCUSSIONS->value)
        ...
        <div class="px-4">
            ...
            >
                <x-slot name="trigger">
        <a data-modal-toggle="add-reply-modal" class="mt-5 w-full bg-transparent hover:cursor-pointer px-3 py-10 border-2 border-transparent border-dashed hover:border-slate-200 rounded text-slate-500 hover:text-slate-700 font-medium text-left">
            Write a reply...
        </a>
                </x-slot>
                <x-slot name="heading">
