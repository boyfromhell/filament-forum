        ...
        @if($likes->count())
            @foreach($likes as $like)
                <div class="w-full flex items-center gap-2 py-3 @if(!$loop->last) border-b border-slate-200 @endif">
                    <img src="{{ $like->user->profile_photo_url }}" alt="Avatar"
                         class="rounded-full w-10 h-10 border border-slate-200 shadow"/>
                    <div class="flex flex-col gap-0">
                        <a href="{{ route('profile.show', ['username' => $like->user->username]) }}" class="hover:cursor-pointer hover:underline text-slate-700 font-medium">{{ $like->user->username }}</a>
