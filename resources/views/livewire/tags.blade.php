<div class="container mx-auto flex items-center flex-wrap">
    @if($tags->count())
        @foreach($tags as $tag)
            <a href="{{ route('forum.tag', ['tag' => $tag, 'slug' => Str::slug($tag->name)]) }}"
               class="lg:w-1/3 w-full p-6 text-white hover:shadow-xl shadow-slate-900 hover:scale-105 transition-all hover:rounded-xl"
               style="background-color: {{ $tag->color }};">
                ...
                <span class="flex justify-between items-center gap-2 items-center text-xs">
                    ...
                    <span>View discussions @if($discussions)
                            ({{ $discussions }})
                        @endif</span>
                    <x-fas-arrow-right-long class="w-4"/>
                </span>
            </a>
        @endforeach
    @else
        <div class="w-full flex flex-col justify-center items-center text-center gap-2 py-20">
            ...
            <a href="{{ route('home') }}"
               class="flex items-center gap-1 text-sm text-blue-500 hover:text-blue-600 px-3">
